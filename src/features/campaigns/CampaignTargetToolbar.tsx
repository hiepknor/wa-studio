...
export function emptyCampaignTargetFilters(): CampaignTargetFilters {
  return {
    capabilityFreshness: [],
    capabilityStatuses: [],
    isActive: undefined,
...
export function activeCampaignTargetFilterCount(filters: CampaignTargetFilters): number {
...
interface CampaignTargetToolbarProps {
  filters: CampaignTargetFilters;
  filtersOpen: boolean;
  inputQuery: string;
  loading: boolean;
  onFiltersChange: (filters: CampaignTargetFilters) => void;
...
export function CampaignTargetToolbar({
  filters,
  filtersOpen,
  inputQuery,
  loading,
  ...
  participantErrors,
  total,
}: CampaignTargetToolbarProps) {
  const filterCount = activeCampaignTargetFilterCount(filters);
...
interface CampaignTargetFilterPanelProps {
  filters: CampaignTargetFilters;
  onChange: (filters: CampaignTargetFilters) => void;
  onClose: () => void;
  onParticipantErrorsClear: () => void;
  participantErrors: ParticipantFilterErrors;
}
...
function CampaignTargetFilterSummary({
  ...
  const hasFilters = activeCampaignTargetFilterCount(filters) > 0;
  return (
    <div aria-label="Selected target group filters" className="data-filter-summary">
      ...
      <Button disabled={!hasFilters} onClick={() => onChange(emptyCampaignTargetFilters())} size="sm" variant="ghost">Clear all</Button>
    </div>
  );
}
