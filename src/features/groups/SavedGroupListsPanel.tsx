...
import type { RuntimeSavedGroupList, RuntimeSavedGroupListPage } from "@/shared/api/runtime-client";
...
interface SavedGroupListsPanelProps {
...
type EditorState =
  ...
  | { kind: "edit"; list: RuntimeSavedGroupList };
...
export function SavedGroupListsPanel({ navigation }: SavedGroupListsPanelProps) {
  const { connected, selectedSessionId } = useRuntimeConnection();
  const toast = useToast();
  if (!connected) throw new Error("SavedGroupListsPanel requires a Runtime connection");
  const api = connected.api;
  const [page, setPage] = useState<RuntimeSavedGroupListPage | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const nextPage = await api.listSavedGroupLists({
  ...
  return (
    <div aria-labelledby="groups-workspace-saved-tab" className="groups-screen saved-group-lists-screen stack stack-lg" id="groups-workspace-saved-panel" role="tabpanel">
      ...
      <div className="data-table-container saved-group-lists-panel">
        <div className="data-table-toolbar saved-group-lists-toolbar">
          <SearchField id="saved-group-lists-search" label="Search saved lists" loading={loading} onChange={changeSearch} placeholder="Search list name or description" value={inputQuery} variant="toolbar" />
        ...
        {error && <InlineAlert action={<Button onClick={() => setReloadRevision((revision) => revision + 1)} size="sm">Retry</Button>} className="data-table-error" title="Could not load saved lists">{error}</InlineAlert>}
        <div aria-busy={loading || undefined} className="data-table-scroll">
          <table>
            <caption>Saved group lists for the active session</caption>
