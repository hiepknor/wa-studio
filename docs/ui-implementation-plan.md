...
Current Campaign Drafts, Search & Preflight baseline: 309.21 kB JavaScript (93.09 kB gzip) and 53.22 kB CSS (10.55 kB gzip), plus 106.41 kB of locally bundled variable-font subsets. The production brand mark is an inlined SVG; native bundle icons are generated from the dedicated SVG app-icon master.
...
Campaign Workspace target/preflight refinement (2026-08-15): `npm run check` passes with 191 tests. Targets now distinguish staged selection from the authoritative saved set, summarize additions/removals, retain canonical state after replacement failures, and expose saved targets through a disclosure that automatically opens on failure. Saved targets outside the current Groups result page can be removed from or restored to the staged selection without writing Runtime until `Save target set`. Target search reuses `SearchField`, trims and debounces by 300 ms, omits whitespace-only queries, and rejects late query, campaign, session, close, and unmount responses. Preflight reuses `SelectMenu` for Dry run/Live policy and exposes one `Run preflight` action; neither mode creates a run or sends a message. Changing workflow tabs resets drawer-body scroll while the header, workflow navigation, and action footer remain stable. The live Tauri app was audited without mutating staging data at 1500×820 docked, 1100×720 overlay, and 820×720 minimum width; focus, menu placement, scroll ownership, footer stability, and responsive layout passed visual inspection.
