...
Campaign Target groups refinement (2026-08-15): Saved lists are an import tool inside the shared target toolbar rather than a separate workspace section. The lazy, searchable popover fetches fresh membership before Add or confirmed Replace and invalidates late responses when closed. One table separates saved/selected rows outside the current Runtime result page from current results without adding a selection-state column; the header checkbox remains explicitly page-scoped. A compact `Saved / Staged / + / −` summary and `Reset to saved` action make local changes recoverable while `Save target set` remains the only Runtime persistence point. `npm run check` passes with 32 test files and 227 tests, including lazy list loading, popover-close race protection, staged reset, shared-table grouping, existing Campaign behavior, TypeScript/Vite production build, and Rust fmt/clippy.

Group List editor refinement (2026-08-15): The editor now separates List details from Groups, uses domain-neutral shared selection styles, keeps saved and staged membership visible in one table, and reports `Saved / Staged / + / −` in the sticky footer. Clean edits disable Save; Reset changes restores canonical metadata and membership without mutation; Archive is isolated from save actions; partial metadata/membership failure copy states exactly what Runtime persisted. Saved-list names open the editor directly and descriptions can use two lines. Live Tauri smoke confirms selection, page-scoped select-all, filters, dirty-close, Reset, sticky footer, and the minimum supported 800px window. `npm run check` passes with 32 test files and 229 tests, TypeScript/Vite production build, and Rust fmt/clippy.
