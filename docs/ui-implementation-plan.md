...
Current Campaign Drafts, Search & Preflight baseline: 304.56 kB JavaScript (91.27 kB gzip) and 48.81 kB CSS (9.91 kB gzip), plus 106.41 kB of locally bundled variable-font subsets. The production brand mark is an inlined SVG; native bundle icons are generated from the dedicated SVG app-icon master.
...
Campaign server-side search/filter validation (2026-08-14): `npm run check` passes with 182 tests, covering 300 ms debouncing, trimmed/omitted queries, comma-separated multi-select filters, filtered pagination and empty states, offset recovery, typed validation errors, session/query/filter/unmount race protection, and the existing Draft/Target/Preflight and target-picker regressions. The Campaign Workspace redesign was inspected in the live Tauri development app at 1100×720 and 760×650 across Details, Targets, and Preflight. The Vite production build and Rust fmt/clippy gates pass. Native packaging and staging smoke testing were not run because deployment was outside this change.
