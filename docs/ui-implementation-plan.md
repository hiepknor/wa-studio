...
Current Campaign Drafts, Search & Preflight baseline: 307.51 kB JavaScript (92.36 kB gzip) and 51.78 kB CSS (10.34 kB gzip), plus 106.41 kB of locally bundled variable-font subsets. The production brand mark is an inlined SVG; native bundle icons are generated from the dedicated SVG app-icon master.
...
Campaign server-side search/filter validation (2026-08-14): `npm run check` passes with 186 tests, covering 300 ms debouncing, trimmed/omitted queries, comma-separated multi-select filters, filtered pagination and empty states, offset recovery, typed validation errors, session/query/filter/unmount race protection, and the existing Draft/Target/Preflight and target-picker regressions. The Campaign Workspace redesign and shared schedule `SelectMenu` were inspected in the live Tauri development app at 1500×850 docked, 1100×720 overlay, and the native 820×650 minimum window. The wide Drawer uses a 500 px docked rail, resets body scroll when its content identity changes, preserves its fixed footer, and disables Targets/Preflight until the draft exists; the menu opens upward near that footer without clipping. The Vite production build and Rust fmt/clippy gates pass. Native packaging and staging smoke testing were not run because deployment was outside this change.
