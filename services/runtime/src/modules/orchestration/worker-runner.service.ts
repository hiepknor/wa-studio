...
export class WorkerRunnerService {
  ...
  async start(): Promise<void> {
    ...
    try {
      workers.push(
        this.queues.startWorker<MessageSendQueuePayload>(
          MESSAGE_SEND_QUEUE,
        ...
        this.queues.startWorker<{ idempotencyKey: string }>(
          WEBHOOK_QUEUE,
        ...
        this.queues.startWorker<FullGatewaySyncPayload | GroupCapabilityRefreshPayload | GroupReconciliationPayload | TargetedGroupReconciliationPayload>(
          GATEWAY_SYNC_QUEUE,
        ...
        this.queues.startWorker<{ runId: string }>(
          CAMPAIGN_QUEUE,
